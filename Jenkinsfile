pipeline {
    ...
    environment {
        ...
        REPO_URL = 'https://github.com/ritesh1603/FastAPI.git'
    }
    ...
    stages {
            ...
            steps {
                script {
                    if (env.BRANCH_NAME == 'dev') {
                        env.DEPLOY_PATH = '/home/vagrant/dev/FastAPIapp'
                        env.BRANCH_TO_BUILD = 'uat'
                    } else if (env.BRANCH_NAME == 'uat') {
                        env.DEPLOY_PATH = '/home/vagrant/uat/FastAPIapp'
                        env.BRANCH_TO_BUILD = 'staging'
                    } else if (env.BRANCH_NAME == 'staging') {
                        env.DEPLOY_PATH = '/home/vagrant/staging/FastAPIapp'
        ...
        stage('Run Ansible Playbook') {
            steps {
                bat """
                "$PLINK_PATH" -pw "$DEPLOY_PASSWORD" "$DEPLOY_USER@$DEPLOY_HOST" "ansible-playbook /home/vagrant/ansible_project/fastapi/${env.BRANCH_NAME}-deployment-playbook.yml"
                """
            }
        }
